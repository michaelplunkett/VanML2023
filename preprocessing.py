...
# Create time columns
calls['DateTimeStart'] = calls.apply(lambda x: dt.datetime.combine(x.DateStart, x.TimeStart), axis=1)
...
calls['TimeofDay'] = calls['DateTimeStart'].apply(lambda x: x.timestamp() - dt.datetime(x.year, x.month, x.day).timestamp())
